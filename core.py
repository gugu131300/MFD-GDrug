...
class Predictor(nn.Module):
    def __init__(self, input_dim, hidden_dim, dropout_rate, n_filters, embed_dim, num_features_xt, output_dim, args):
        ...
        self.conv_xt_1 = nn.Conv1d(in_channels=400, out_channels=n_filters, kernel_size=7)
    ...
    def forward(self, batch, esm_embedding, cnn_embedding, mol2vec_embedding):
        ...
        # comb
        combined_fea = torch.cat((compound, combined_pro), 1)  # 在列方向上叠加###########################
        # model
        d_block = self.dropout(self.relu(self.out_fc1(combined_fea)))
        out = self.dropout(self.relu(self.out_fc2(d_block)))
        out2 = self.out_fc3(out)
        out3 = self.out_fc4(out2).squeeze()
        return out3, out2
